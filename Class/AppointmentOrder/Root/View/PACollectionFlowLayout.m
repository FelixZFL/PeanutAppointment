...
@interface PACollectionFlowLayout()
...
@implementation PACollectionFlowLayout
...
- (void)setItemFrameWithIndexPath:(NSIndexPath *)indexPath {
    ...
    /** 保存在数组中 */
    while (self.itemsattributes.count <= indexPath.section) {
        [self.itemsattributes addObject:[NSMutableArray array]];
    }
...
- (CGFloat)countItemSizeWithIndexPath:(NSIndexPath *)indexPath {
    ...
    return MAX(MIN(size.width + 24.0, self.viewWidth), self.itemHeight);
}
