...
+ (void)stsWithAppUid:(NSString *)appUid success:(void (^)(AlivcSts *sts))success failure:(void(^)(NSString *errString))failure{
    ...
    //type：1视频    2直播
    [YQNetworking postWithApiNumber:API_NUM_20023 params:@{@"type":@"2"} successBlock:^(id response) {
        if (getResponseIsSuccess(response)) {
            STSInfoModel *model = [STSInfoModel mj_objectWithKeyValues:getResponseData(response)];
            // 存储
            [[NSUserDefaults standardUserDefaults] setObject:model.accessKeyId forKey:AlivcAppServer_StsAccessKey];
            [[NSUserDefaults standardUserDefaults] setObject:model.accessKeySecret forKey:AlivcAppServer_StsSecretKey];
            [[NSUserDefaults standardUserDefaults] setObject:model.securityToken forKey:AlivcAppServer_StsSecurityToken];
            [[NSUserDefaults standardUserDefaults] setObject:model.expiration forKey:AlivcAppServer_StsExpiredTime];
            [[NSUserDefaults standardUserDefaults] synchronize];
            
            AlivcSts *sts = [[AlivcSts alloc] init];
            sts.accessKey = model.accessKeyId;
            sts.secretKey = model.accessKeySecret;
            sts.securityToken = model.securityToken;
            sts.expireTime = model.expiration;
