...
NSString *getResponseMsg(id responseObject){
    return [NSString stringWithFormat:@"%@",responseObject[@"msg"]];
}

NSInteger getResponseCode(id responseObject){
...
/*
 errorCode = 500;
 errorMsg = "Content type 'application/x-www-form-urlencoded;charset=UTF-8' not supported";
 fieldErrors =     (
 );
 url = "http://test.wxzhou.com/api/login";
 msg = "\U7528\U6237\U8ba4\U8bc1\U5931\U8d25";
 result = "<null>";
 status = 500;
 url = "<null>";
    ...
    session = [manager GET:url
                parameters:params
                  progress:^(NSProgress * _Nonnull downloadProgress) {
                      ...
                      NSLog(@"\nURL===%@ \n responseObject====%@",session.currentRequest.URL,responseObject);
...
+ (YQURLSessionTask *)postWithApiNumber:(NSString *)number
                                 params:(NSDictionary *)params
                           successBlock:(YQResponseSuccessBlock)successBlock
                              failBlock:(YQResponseFailBlock)failBlock {
    return [self postWithUrl:[PATool getFullURLByNum:number]
       refreshRequest:NO
                cache:NO
               params:[PATool addKeyAndNum:number toParams:params]
        progressBlock:nil
    ...
    session = [manager POST:url
                 parameters:params
                   progress:^(NSProgress * _Nonnull uploadProgress) {
                       ...
                       //判断新版本更新
                       NetworkResponseModel *resp = [NetworkResponseModel mj_objectWithKeyValues:responseObject];
